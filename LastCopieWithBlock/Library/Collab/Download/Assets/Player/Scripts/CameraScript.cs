...
public class CameraScript : MonoBehaviour
{
    ...
    //Target
    public Transform unlocked_lookAt;
    public Transform locked_lookAt;
    bool lockOn = false;
    ...
    void CameraUpdater()
    {
        Transform target = unlocked_lookAt.transform;
    ...
    void LockedCameraTurning()
    {
        //Vector3 dir = transform.position - lookingAt.position;
        Quaternion targetRotation = Quaternion.LookRotation(locked_lookAt.position - transform.position);
        // Smoothly rotate towards the target point.
        transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, 10f);
        //transform.position = new Vector3((int)transform.position.x, (int)transform.position.y + 2, (int)transform.position.z);
    }
    ...
    {
        float ang = Vector3.SignedAngle(unlocked_lookAt.transform.forward, transform.forward, Vector3.up);
        if (ang > 0)
        {
            ang = 180 - ang;
